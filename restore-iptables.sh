...
echo "Note: The related ip6ables ruleset will also be restored"
...
ipv6_Selected_rule=$(ls "$saveRulesDir" | grep -F "ip6tables.rules" | grep -Fv "unshare" | sed -n $ruleAnswer\p)
echo
echo "This will restore the rule sets: \"$selected_rule\" and \"$ipv6_Selected_rule\""
echo "Are you sure? Y/N"
read answer
if [[ $answer == "Y" || $answer == "y" || $answer = "" ]]; then
	iptables-restore < "$saveRulesDir"/"$selected_rule"
	ip6tables-restore < "$saveRulesDir"/"$ipv6_Selected_rule"
	if [ $? -eq 0 ]; then
		saveTables
		if [ $? -eq 0 ]; then
			echo
